...
# Every published build must have matching, successfully processed symbols.
DSYM_SRC="$(dirname "$APP_SRC")/$(basename "$APP_SRC" .app).dSYM"
scripts/upload-debug-symbols.sh "$APP_SRC" "$DSYM_SRC" || fail "debug symbols are required for release"
