...
python3 - "$FILE" "$SESSION" "$SHORT" "$CALLSIGN" "$TODAY" <<'PY' 2>/dev/null || true
import json, sys
...
snippet = (
    ...
    '  <div style="flex:1;min-width:220px">Created by <b>{callsign}</b> '
    '&middot; session <b>{short}</b> &middot; {today}</div>\n'
    ...
    '</footer>'
).format(callsign=callsign, short=short, today=today, session=session, path=path)
